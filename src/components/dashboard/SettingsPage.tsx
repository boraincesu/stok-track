  ...
  return (
    <section className="flex max-w-4xl flex-col gap-8">
      <header>
        ...
        <p className="text-sm text-text-light-secondary dark:text-dark-secondary">Manage your profile, alerts, and workspace preferences.</p>
      </header>
      ...
      <article className="rounded-2xl border border-border-light dark:border-border-dark bg-card-light dark:bg-card-dark p-6 shadow-sm">
        <h3 className="text-lg font-semibold text-text-light-primary dark:text-dark-primary">Profile Information</h3>
        <div className="mt-6 flex flex-col gap-8 md:flex-row">
          <div className="relative">
            <div
              className="size-24 rounded-full bg-cover bg-center"
              style={{ backgroundImage: 'url("https://i.pravatar.cc/120?img=12")' }}
            />
            <button
              type="button"
          ...
          <div className="grid flex-1 grid-cols-1 gap-6 md:grid-cols-2">
            ...
            },
            {
              ...
              <label key={field.label} className="flex flex-col gap-2 text-sm font-semibold text-text-light-primary dark:text-dark-primary">
                {field.label}
                <input
                  className={`rounded-lg border border-border-light dark:border-border-dark bg-background-light dark:bg-background-dark px-3 py-2 text-sm font-normal text-text-light-primary dark:text-dark-primary outline-none transition focus:border-primary focus:ring-1 focus:ring-primary ${
                    field.disabled ? "cursor-not-allowed text-text-light-secondary" : ""
                  }`}
      ...
      <article className="rounded-2xl border border-border-light dark:border-border-dark bg-card-light dark:bg-card-dark p-6 shadow-sm">
        <h3 className="text-lg font-semibold text-text-light-primary dark:text-dark-primary">Notifications</h3>
        <div className="mt-6 space-y-4">
          {[
            ...
            <div key={item.title} className={`flex items-center justify-between py-3 ${index === 0 ? "" : "border-t border-border-light dark:border-border-dark"}`}>
              <div>
                <p className="font-medium text-text-light-primary dark:text-dark-primary">{item.title}</p>
                <p className="text-sm text-text-light-secondary dark:text-dark-secondary">{item.description}</p>
              </div>
              <label className="relative inline-flex cursor-pointer items-center">
                <input defaultChecked={item.defaultChecked} type="checkbox" className="peer sr-only" />
